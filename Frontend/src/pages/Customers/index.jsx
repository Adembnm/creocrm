...
const Customers = () => {
  ...
  const { customers, pageCount, currentPage, totalFiltredCustomers } =
  ...
  return (
    <div className="m-2 md:m-6 p-2 md:p-6">
      <div className="flex justify-between items-center mt-20 sm:mt-0">
        ...
        >
          {totalFiltredCustomers && <b>{totalFiltredCustomers}</b>}{' '}
          {totalFiltredCustomers === 0
            ? t('customers_value_min')
            : totalFiltredCustomers === 1
            ? t('customers_value')
            : totalFiltredCustomers > 1 && totalFiltredCustomers < 11
            ? t('customers_value_min')
            : t('customers_value')}
        </p>
        <AiOutlineUserAdd
          className="text-3xl	cursor-cell"
          style={{ color: currentColor }}
      ...
      <div className="flex justify-end items-center py-4">
        <input
          type="text"
          className="w-1/4 px-4 py-3 bg-gray-100 focus:outline-none"
      ...
      <div className="table_container">
        <table className="w-full rounded-md table-auto">
          <thead className="border border-gray-100 bg-gray-100">
            <tr>
              <th className="hidden sm:table-cell px-2 sm:px-4 py-3 text-sm">
                {t('avatar')}
              </th>
              <th
              ...
              <th
                className={`hidden sm:table-cell px-2 sm:px-4 py-3 text-sm ${
                  i18n.language === 'ar' ? 'text-right' : 'text-left'
                }`}
              >
                {t('phone')}
          ...
          <tbody>
            {customers?.map((customer) => (
              <tr
                key={customer._id}
                className="cursor-pointer transition-all duration-800 hover:bg-slate-400 dark:hover:bg-slate-500"
              >
                <td className="hidden sm:table-cell border border-gray-100 dark:border-gray-500 px-2 sm:px-4 py-1">
                  <img
                    src={customer.avatar || '/images/user.png'}
                    alt="Avatar"
                    className="block mx-auto w-12 h-12 rounded-full border-2 border-slate-200 object-cover"
                  />
                </td>
                <td className="text-sm border border-gray-100 dark:border-gray-500 px-2 sm:px-4 py-1 dark:text-slate-100">
                  {customer.name}
                </td>
                <td
                  className={`hidden sm:table-cell text-sm border border-gray-100 dark:border-gray-500 px-2 sm:px-4 py-1 dark:text-slate-100`}
                  style={{
                    direction: 'ltr',
                ...
                >
                  {customer.birthDay
                    ? new Date(customer.birthDay).toISOString().substring(0, 10)
                    : ''}
                </td>
                <td className="border border-gray-100 dark:border-gray-500 px-2 sm:px-4 py-1 flex justify-center items-center h-full">
                  <AiOutlineEdit
                    className="mx-1 my-3 text-2xl text-green-500 hover:text-green-800 cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
                  ...
                  />
                  <AiOutlineDelete
                    className="mx-1 my-3 text-2xl text-red-500 hover:text-red-800 cursor-pointer"
                    onClick={(e) => {
                      e.stopPropagation();
