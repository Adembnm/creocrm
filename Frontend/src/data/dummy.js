...
import { AiOutlineSetting, AiOutlineUserSwitch, AiOutlineProject, AiOutlineCalendar, AiOutlineShoppingCart, AiOutlineAreaChart, AiOutlineBarChart, AiOutlineStock, AiOutlineTeam } from 'react-icons/ai';
import { FiShoppingBag, FiEdit, FiPieChart, FiBarChart, FiCreditCard, FiStar, FiShoppingCart } from 'react-icons/fi';
import { BsKanban, BsBarChart, BsBoxSeam, BsCurrencyDollar, BsShield, BsChatLeft, BsBox } from 'react-icons/bs';
import { MdDashboard, MdOutlineSettingsSuggest, MdOutlineDashboard, MdOutlineSupervisedUserCircle } from 'react-icons/md';
import { BiColorFill, BiTask, BiUserVoice, BiUserX, BiUserMinus } from 'react-icons/bi';
import { IoMdContacts } from 'react-icons/io';
import { RiContactsLine, RiStockLine, RiMessengerLine, RiUserStarLine } from 'react-icons/ri';
import { MdOutlineSupervisorAccount, MdOutlineLocalShipping, MdOutlineCategory, MdEvent } from 'react-icons/md';
...
export const settingsData = {
  organization_name: 'CREO SYSTEM',
  organization_logo: '/logo.png',
  invoices_logo: '/creo.png',
  organization_address: 'I.Jeamy E.1 B.1, Rue Constatine, La Corniche,4000 Sousse, Tunis',
  organization_city: 'Sousse',
  organization_phone: '+216 50 870 256 / +216 25 788 950',
  organization_email: 'contact@creo.tn',
  organization_website: 'https://creo.tn',
  organization_rib: 'TN 00000 0000 0000 0000 0000',
  fiscal_number: '123456789',
  organization_currency: 'TND',
  company_services_tax_factor: 19,
  tax_name: 'TVA',
  quost_reference_prefix: 'D-',
  order_reference_prefix: 'F-',
...
export const sideMenu = (role) => {
  if (role === 1) {
    return [
      {
        title: 'CRM',
        status: settingsData.modules?.find(m => m.name === 'CRM')?.status,
        prelink: "/crm",
        links: [
          {
            name: 'home',
          ...
          },
          {
            name: 'customers',
            icon: <RiContactsLine />,
            status: settingsData.modules?.find(m => m.name === 'CRM')?.links?.find(l => l.name === 'Customers')?.status
          },
          
          {
            name: 'orders',
            icon: <AiOutlineShoppingCart />,
            status: settingsData.modules?.find(m => m.name === 'CRM')?.links?.find(l => l.name === 'Orders')?.status
          },
          ,
          {
            name: 'payments',
            icon: <BsCurrencyDollar />,
            status: settingsData.modules?.find(m => m.name === 'CRM')?.links?.find(l => l.name === 'Payments')?.status
          },
          {
            name: 'appointments',
            icon: <AiOutlineCalendar />,
            status: settingsData.modules?.find(m => m.name === 'CRM')?.links?.find(l => l.name === 'Appointments')?.status
          },
          {
            name: 'services',
            icon: <AiOutlineSetting />,
            status: settingsData.modules?.find(m => m.name === 'CRM' && m.type === 'Services')?.links?.find(l => l.name === 'Services')?.status
          },
        ...
        links: [
          {
            name: 'users',
            icon: <AiOutlineTeam />,
            status: settingsData.modules?.find(m => m.name === 'SYSTEM')?.links?.find(l => l.name === 'Users')?.status
          },
...
//Get Customer Rating Range
export const getCustomerRatingRange = [
  {
    id: 1,
    value: 1,
    min: 0,
...
export const getSuppliersList = [
  {
    id: 1,
    logo: "https://scontent.ftun15-1.fna.fbcdn.net/v/t1.6435-9/43952883_2335178089829978_7914684566776840192_n.jpg?_nc_cat=109&ccb=1-7&_nc_sid=8bfeb9&_nc_ohc=OFK_2yjqq9cAX-rwHWC&_nc_ht=scontent.ftun15-1.fna&oh=00_AT_aSe3xUhClbMe6ytgseKMy3hX-ycL8skHhVnkAn4TDYw&oe=63013231",
    name: 'Ahmed Grissa',
    phone: '0123456789',
    email: 'ahmed.grissa@email.com',
    address: 'Msaken',
    supllier_bonus: 25,
...
// Shippers List
export const getShippersList = [
  {
    id: 1,
    logo: "https://www.droppex.com/img/logodroppex.png",
    name: 'Dropex',
    phone: '0123456789',
    email: 'dropex@email.com',
    shpping_tax: 7.5,
    return_tax: 4,
    swap_tax: 4,
  },
...
export const kanbanGrid = [
  {
    headerText: 'Negotiation',
    keyField: 'Negotiation',
    allowToggle: true,
    color: "#4f6bab",
    icon: <BiUserVoice size={25} color='#4f6bab' />,
  },

  {
    ...
    allowToggle: true,
    color: "#7eb469",
    icon: <AiOutlineUserSwitch size={25} color='#aca440' />
  },

  {
    ...
    allowToggle: true,
    color: "#aca440",
    icon: <BiUserX size={25} color='#aca440' />
  },
  {
    headerText: 'Lost',
    keyField: 'Lost',
    ...
    isExpanded: false,
    color: "#914542",
    icon: <BiUserMinus size={25} color="#914542" />
  },
